...
export function matchInBrackets(view: EditorView, inclusive: boolean) {
	...
	for (let seli = 0; seli < view.state.selection.ranges.length; seli++) {
		...
		for (let i = pos - 1; i >= 0; i--) {
			...
			if (
				MATCHEABLESTARTS.includes(doc[i]) ||
				MATCHEABLEDUOS.includes(doc[i])
			) {
				if (depth === 0) {
					startPos = i + 1;
					startChar = doc[i];
...
function selectParagraph(view: EditorView) {
	...
	const newRanges = [];
	for (let seli = 0; seli < view.state.selection.ranges.length; seli++) {
		const lineNum = state.doc.lineAt(
			view.state.selection.ranges[seli].from,
		).number;
...
function selectWord(view: EditorView) {
	...
	const newRanges = [];
	for (let seli = 0; seli < state.selection.ranges.length; seli++) {
		const range = state.selection.ranges[seli];
		const word = state.wordAt(range.from);

		let startPos = range.from;
		let endPos = range.to;
...
export function openLine(view: ViewLike, below: boolean) {
	...
	if (below) {
		...
		from = line.to;
		insertText = view.state.lineBreak + indent;
		...
		from = line.from;
		insertText = indent + view.state.lineBreak;
