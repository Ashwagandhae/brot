...
import { type ModeState, SearchMode } from "./entities";
...
export class CommandPanel implements Panel {
	...
	private updateVisibility() {
		...
		const hasInput = this.inputContainer.childElementCount > 1;

		const hasMinorCommand = Boolean(this.minorCommand.textContent?.trim());

		const hasContent = hasMessage || hasInput || hasMinorCommand;

		if (hasContent) {
			this.dom.classList.add("cm-panel-open");
			$style(this.dom, {
				height: "",
				padding: "",
				margin: "",
			...
			this.dom.classList.remove("cm-panel-open");
			$style(this.dom, {
				height: "0px",
				padding: "0px",
				margin: "0px",
