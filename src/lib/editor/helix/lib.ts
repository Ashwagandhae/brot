	...
	return {
		...
		onInput(input_: string) {
			...
			if (match.done) {
				...
				const matchedSelection = EditorSelection.range(
					match.value.from,
					match.value.to,
				);
...
const helixCommandBindings: {
	...
	normal: {
		...
		["N"](view, mode) {
			...
			const result = backwardsSearch(
				view.state,
				query,
				mode,
				(matches, main) => {
					let selection: EditorSelection | SelectionRange =
...
export function helix(options: Options = {}): Extension {
	...
	return [
		...
		defaultYankRegister.of(
			options.config?.["editor.default-yank-register"] ?? `"`,
		),
